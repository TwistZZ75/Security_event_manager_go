...
interface AgentMiniModalProps {
  /** hostname или pc_name из события */
  hostname: string;
  onClose: () => void;
}
...
export default function AgentMiniModal({ hostname, onClose }: AgentMiniModalProps) {
  ...
  useEffect(() => {
    ...
    // Пробуем найти агента сначала по точному hostname,
    // затем — без домена (ubuntu-vm.local → ubuntu-vm)
    const tryLoad = async () => {
      try {
        const data = await getAgent(hostname);
        setAgent(data);
      } catch {
        // Попробуем короткое имя
        const short = hostname.split('.')[0];
        if (short && short !== hostname) {
          try {
            const data = await getAgent(short);
            setAgent(data);
            return;
          } catch {/* fall through */}
        }
  ...
  const fields: [string, string][] = agent ? [
    ...
    ['Registered',     fmtDate(agent.RegisteredAt)],
    ['Last Seen',      fmtDate(agent.LastSeen)],
  ] : [];
  ...
  return (
    <div
      ...
      style={{
        ...
        padding: '60px 20px 40px', overflowY: 'auto', outline: 'none',
      }}
    >
      <div
      ...
      >
        {/* Header */}
        <div style={{
          padding: '16px 20px', borderBottom: '1px solid var(--navy-border)',
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
            ...
            {agent && (
              <span style={{
                ...
                color: isOnline ? 'var(--mint)' : '#888bac',
              }}>
            ...
            <div style={{
              ...
              <div>
                ...
                <div style={{ fontSize: '11px', color: 'var(--text-secondary)' }}>
                  Убедитесь, что агент зарегистрирован и hostname совпадает с pc_name в событии.
                </div>
              </div>
            </div>
          )}
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px', marginBottom: '14px' }}>
                {fields.map(([label, value]) => (
                  <div key={label} style={{ background: 'var(--navy)', borderRadius: '8px', padding: '10px 12px' }}>
                    <div style={{ fontSize: '10px', color: 'var(--text-secondary)', fontFamily: 'var(--font-display)', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.07em', marginBottom: '4px' }}>
                      {label}
                    </div>
                    <div style={{ fontSize: '12px', fontFamily: 'var(--font-mono)', color: 'var(--mint)', wordBreak: 'break-all' }}>
                      {value}
                    </div>
                  </div>
                ))}
        ...
        <div style={{ padding: '10px 20px', borderTop: '1px solid var(--navy-border)', display: 'flex', alignItems: 'center', gap: '6px', color: 'var(--text-secondary)', fontSize: '11px', fontFamily: 'var(--font-mono)' }}>
          <Clock size={11} />
          Last seen: {agent ? fmtDate(agent.LastSeen) : '—'}
        </div>
      </div>
    </div>
  );
